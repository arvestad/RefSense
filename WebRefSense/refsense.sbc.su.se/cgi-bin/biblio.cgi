...
use lib "/var/www/refsense.sbc.su.se/lib/";
use PubMed;
use PMCache;
use Citation;
...
} elsif ($mode eq 'bibtex' || $mode eq 'text' || $mode eq 'html' || $mode eq 'htmlpage') {
...
sub present_refs {
  ...
  my $incoming=$q->param('id');
  if (defined $incoming) {
      my @urlpmid = split(/,/, $incoming);
      if (scalar(@urlpmid) > 0) {
	  $pmids = \@urlpmid;
      }
  }
...
#    $bib = $q->start_pre();
	...
	$bib .= make_text_citation($art_h->{$id}) . "\n";
      } elsif ($mode eq 'html' || $mode eq 'htmlpage') {
...
#    $bib .= $q->end_pre();
  ...
  my $page = '';
  if ($mode eq 'htmlpage') {
    $page = $q->header(-type => 'text/html')
      . $q->start_html(-title => 'References')
      . $bib
      . $q->end_html();
  } else {
    $page = $q->header(-type => 'text/plain')
      . $bib;
  }
  print $page;
}
...
$journalsortlink";
    ...
    $htmllink = $q->div({-class => 'meny'},
			$q->a({-href => $q->self_url}, 'HTML snippet'));

    $q->param('mode', 'htmlpage');
    $htmlpagelink = $q->div({-class => 'meny'},
			$q->a({-href => $q->self_url}, 'HTML page'));
...
sub article_button {
  ...
  if (defined $url) {
      ...
      my $html = CGI::span({-class => 'articlebutton',
			    -id => "${bstr}Button"},
			   CGI::a($linkdata, "&rsaquo;$bstr"));
#			   CGI::a($linkdata, "&rsaquo;$bstr"));
      return $html;
  } else {
      return CGI::span({-class => 'inactivearticlebutton',
			-id => "${bstr}Button"},
		       "&rsaquo;$bstr");
  }
}
...
sub art2html {
  ...
  my $doi = '';
  $elem = $art->{'AID'};
  my $connotealink = article_button('Connotea', undef, undef); # Fake empty button
  foreach my $el (@$elem) {
      if ($el =~ m/\s*(\S+)\s+\[doi\]\s*/) {
	  my $doi = $1;
	  $connotealink = article_button('Connotea', 
					 $Connotea . '/add?continue=confirm&uri=http://dx.doi.org/doi:' .$doi,
					 'true');
      }
  }
#   my $connotealink = article_button('Connotea',
# 				   $Connotea . '/data/add?continue=return' . $doi . uri_escape($art->{'TI'}),
# 				   undef);
