...
#       if (length($year) == 4) {
# 	$key .= substr($year, 2, 2);
#       } else {			# Don't really know why this would happen...
#       }
    }
...
sub interpret_author {
  ...
  if ( $au=~/^et[\s\.]al\.?$/i ) {
    ...
    if ($name[0]=~s/([A-Z])([A-Z]+)/$1\L$2\E/g ) {
      $warning .= "Recapitalizing name from capital letters into into $name[0]. \n";
    }
    if (defined $name[1]) {
      $name[1]=~s/([A-Z][a-z]?)/$&\./g;
    }
...
#    if (length($astr) > $MaxAuthorStringLength) {
    if ($MaxNAuthors != 0 && scalar(@$elem) > $MaxNAuthors) {
      $astr = $elem->[0] . ' et al';
    } else {
      $astr = join(', ', @$elem);
    }
